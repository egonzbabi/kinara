...
- **Dirección de origen en Skydropx incorrecta.** `SKYDROPX_ORIGIN_STREET1` es literalmente el placeholder `"Pendiente de confirmar"` (nunca se cargó la calle real). Al comprar una guía de prueba (tarea 028) ya se había visto que la guía generada usaba esta dirección incompleta. En la tarea 030 se encontró evidencia de que esto podría estar relacionado con que algunas cotizaciones de Skydropx se queden atoradas sin completar nunca (`requires_origin_verification: true` en todas las cotizaciones probadas, exitosas y atoradas por igual — no se pudo confirmar causalidad exacta, pero es sospechoso). Hay que entrar a `pro.skydropx.com` → Direcciones (o Configuración) y cargar la dirección de origen real antes de pasar a producción.
...
- **El sandbox de Skydropx es poco confiable — el fallback de $150 en `/checkout` puede seguir apareciendo a veces incluso con todo bien configurado.** (tarea 030): se confirmó que las variables `SKYDROPX_*` faltaban por completo en Vercel Production (por eso el fallback aparecía SIEMPRE) y ya se agregaron ahí con las credenciales de sandbox + se redesplegó. Pero después de eso, el mismo CP que había funcionado siguió cayendo al fallback en algunas pruebas — el sandbox a veces tarda de más o se queda atorado sin completar la cotización (confirmado probando directo contra la API, tanto local como en producción), y para algunos CP de ciudades reales (Guadalajara, Mérida, Puebla) el sandbox no devuelve ninguna tarifa. Esto no es un bug del código — debería mejorar mucho al pasar a `pro.skydropx.com` con credenciales de producción, pero conviene volver a probar varias ciudades ahí antes de lanzar.
- ~~Formulario de contacto sin correo configurado~~ — **resuelto** (2026-09-15): `RESEND_API_KEY`, `CONTACT_EMAIL_TO` (`contacto@kinarafit.com.mx`) y `CONTACT_EMAIL_FROM` ya están cargados en Vercel Production y el dominio `kinarafit.com.mx` está verificado en Resend. Probado en vivo mandando un mensaje real por `/contacto`: `email_sent: true`. Esto también desbloquea el correo de confirmación de pedido (tarea 037) y el del código de descuento de bienvenida (tarea 070), que usan el mismo `RESEND_API_KEY`.
- **Google Search Console: pedirle a Google que indexe el sitio.** Ya existe `/sitemap.xml` (generado en vivo desde el catálogo real) y `/robots.txt` (tarea 003, SEO técnico) apuntando a él, pero eso no basta por sí solo — hay que darlo de alta en [search.google.com/search-console](https://search.google.com/search-console): agregar la propiedad del dominio (`www.kinarafit.com.mx`, una vez conectado en Vercel), verificar la propiedad (DNS o meta tag), y enviar la URL del sitemap (`https://www.kinarafit.com.mx/sitemap.xml`) para que Google lo revise. Sin esto, Google puede tardar mucho más en encontrar el sitio por su cuenta. Requiere una cuenta de Google del usuario — Claude no puede darlo de alta.
