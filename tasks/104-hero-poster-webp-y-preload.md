...
- 2026-09-13 (más tarde): Confirmado con Lighthouse real contra producción — LCP bajó de 4.6s a 3.4s (Performance 83→89/86) y `modern-image-formats`/`uses-responsive-images` pasan a 100. **Sigue sin cumplir el objetivo de 2.5s** — la causa restante (el loader de `_index.tsx` bloqueaba el envío del HTML esperando el catálogo completo) se investigó y se intentó resolver en la tarea 105 (streaming con `Suspense`/`Await`), pero esa tarea se revirtió: el fix técnico funcionó (el documento pasó de tardar 2.36s a ~0.8s) pero el LCP no mejoró de forma clara en la práctica y las corridas de Lighthouse contra producción se volvieron más ruidosas (73-88 de score, 3.8-4.8s de LCP) sin una tendencia consistente hacia abajo — decisión explícita del usuario de revertir en vez de seguir invirtiendo tiempo en un cambio que no demostraba beneficio real. El código de esta tarea (104) se queda tal cual, como el estado final aceptado: LCP en ~3.4s, por encima del objetivo pero con una mejora sustancial y verificada frente al punto de partida (4.6s→3.4s, y muy por debajo del 5.4s original antes de la tarea 100).
