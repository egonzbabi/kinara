...
- 2026-09-17 (ajuste): El usuario pidió quitar el botón "Ver Mujer" — queda solo "Comprar la colección".
- 2026-09-17 (calidad del video): El usuario pidió mejorar la calidad del video. Diagnóstico: `hero-video.mp4` estaba codificado a solo **681 kbps** a 1080×1080/30fps (bajo a propósito en tareas anteriores para que cargara rápido dentro de la tarjeta chica que tenía antes) — al pasar a full-bleed (tarea 115, este mismo cambio) esa compresión se hizo mucho más visible al ocupar toda la pantalla. Se descargó el archivo original de Supabase Storage, se re-codificó con `ffmpeg` (`libx264 -preset slow -crf 20`, misma resolución 1080×1080, sin tocar el encuadre/recorte) a **1105 kbps** (2.4MB, antes 1.5MB) y se volvió a subir al mismo path (`site/hero-video.mp4`) — no requiere ningún cambio de código porque la URL no cambia. Verificado que el archivo nuevo ya se sirve desde esa URL (`content-length: 2426325`) y que el video sigue reproduciendo bien en el navegador.
