...
- [x] `product_variants.color_name` debe ser el mismo string exacto (mismo formato/espaciado/mayúsculas) para el mismo color a través de todos los productos — nunca crear una variante de formato nueva (ej. "Azulgris", "P.De Rosa", "Rojo-Vino") sin antes buscar si ya existe un nombre para ese color con otro formato. Antes de dar de alta un color nuevo, comparar su `color_hex` contra los nombres existentes — si el hex coincide (o casi coincide) con uno ya usado, reusar ese nombre exacto, no crear uno nuevo. Nombres inconsistentes duplican el swatch del filtro de `/tienda` con círculos casi idénticos y rompen el filtrado (un producto con "Azul Gris" no aparece si el usuario filtra por "Azulgris"). — (origen: tarea 022)
- [x] Al recalcular `color_hex` en bloque para muchos productos a la vez (no foto por foto a mano), es aceptable un muestreo automatizado (recorte central + cuantización + descarte de fondo/piel) siempre que incluya una validación de plausibilidad por nombre (ej. rechazar "Negro" si sale claro, o cualquier nombre no-piel si sale con pinta de piel) que deje el hex anterior sin tocar cuando el resultado no pasa la validación — y que se verifique con una muestra visual (foto vs. swatch) antes de aplicar a toda la base de datos, no solo confiar en el algoritmo a ciegas. — (origen: tarea 022)
...
- [x] Un color de producto puede tener **varias** fotos en `product_images` (varias filas con el mismo `color_name`, `position` = 0, 1, 2...) — no solo una. La de `position = 0` es siempre la principal (la que se ve en la miniatura de color y como foto principal); el resto se muestra en el carrusel de "más fotos" de la página de producto. Cualquier código que lea/escriba fotos por color debe soportar arrays, no asumir una sola foto (`AdminColorInput.imageUrls: string[]`, `Product.colorGallery?: Record<string, string[]>`). — (origen: tarea 019)
...
- [x] No existe envío gratis ni devoluciones gratuitas: no promocionar ningún umbral de "envío gratis" (ni un componente tipo medidor/progreso de carrito) ni "devoluciones gratis en N días" en ningún copy del sitio — el envío siempre se cobra según la cotización real de Skydropx y no se aceptan devoluciones. Confirmado explícitamente por el usuario. — (origen: tarea 008)
