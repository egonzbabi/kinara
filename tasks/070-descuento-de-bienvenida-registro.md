...
- **Prueba de punta a punta real** (post-lanzamiento de `RESEND_API_KEY`/dominio verificado): registro real desde el formulario del home → checkout real con el código (SCULPT SET, $949 + envío $109.98) → pago real completado con tarjeta de prueba de Stripe. Confirmado en Stripe: descuento de $105.90 (10% del total con envío incluido, ya que el envío es una línea más del carrito), total cobrado $953.08. Confirmado en Supabase: pedido creado con `discount_code` correcto, código marcado como usado, stock descontado correctamente. Los 3 casos de rechazo (código ya usado, correo distinto al de registro, subtotal de productos bajo $799) fallan con el mensaje correcto cada uno. Todos los datos de prueba (pedido, 3 registros de correo, stock) se revirtieron al terminar — no queda nada de prueba en la base real.
- El campo de código en `/checkout` sí se verificó en el navegador en esta ronda (contradice la nota anterior sobre no haberse podido probar visualmente) — funciona correctamente end-to-end.
...
- 2026-08-26: `RESEND_API_KEY` configurado y dominio `kinarafit.com.mx` verificado en Resend (fuera de esta tarea, en la conversación) — se aprovechó para probar la tarea 070 completa de punta a punta con una compra real de prueba (ver Pruebas manuales). El usuario pidió aclarar en el mensaje del mínimo de $799 que es "sin contar el envío" — se actualizó en los 3 lugares donde aparece: el banner del home, el correo de bienvenida, y el mensaje de error si el código no alcanza el mínimo.
- 2026-08-26: El usuario pidió que, al registrarse, también se le indique que use el código al momento de pagar (no solo que "revise su correo"). Se actualizó el mensaje de éxito en `WelcomeDiscountBanner.tsx` (ambas variantes, con y sin correo enviado) para agregar la instrucción de ingresarlo en el checkout al pagar. Verificado con `npm run typecheck` y una llamada directa a `/api/newsletter-signup` (el panel del navegador de esta sesión seguía fallando — captura en blanco y refs inválidos — así que se confirmó la respuesta `{ok:true, emailSent:false}` que dispara la rama correcta del texto, en vez de forzar la UI rota).
